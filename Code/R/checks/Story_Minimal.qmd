...
```{r}
#| label: Load Spectra Data and results 1g min 
...
```{r}
#| label: median weight >16g



# # Do them separately
# wigley_16g_season_qs <- bind_rows(
#     mutate(wigley_in_16, 
...
#  ggplot() +
#   geom_line(
#     data = filter(wigley_16g_season_qs, Percentile == "50%"),
#     aes(x = est_year, y = kg_roll, color = season),
#     alpha = 0.6) +
#   scale_color_gmri() +
...
#   facet_grid(area~.) +
#   labs(
#     y = "Median Body Weight", 
#     x = "Year", 
#     fill = "Community",
#     title = "Median Weight, Individuals Over 16g")
...
```{r}
#| label: data wrangling for dissimilarity
...
```{r}
#| label: year 1 dissimilarity plot
# Plot the beta diversity change relative to y1
...
```{r}
# Plot the abundance based metrics
dissim_long %>% 
  filter(str_detect(type, "bray")) %>% 
  ggplot() +
    geom_point(
      aes(x = as.numeric(year), y = value, color = season), 
      size = 0.5, alpha = 0.6) +
    geom_ma(aes(x = as.numeric(year), y = value, color = season), linetype = 1, 
      n = 5, ma_fun = SMA, key_glyph = "timeseries" ) +
    scale_color_gmri() +
    facet_nested(
      area~type*metric, scales = "free", 
      labeller = label_wrap_gen(width = 14)) +
    labs(
      y = "Bray-Curtis Dissimilarity\nfrom Abundance Matrices",
      x = "Year", color = "Season")
...
```{r}
# Plot the presence-absence versions
dissim_long %>% 
  filter(str_detect(type, "bray")) %>% 
  ggplot() +
    geom_point(
...
# Just plot the change vs. 1970
bray_plot <- dissim_long %>% 
  filter(
    metric == "dissimilarity total compare first year",
    str_detect(type, "bray")) %>% 
  ggplot() +
    geom_point(
      aes(x = as.numeric(year), y = value, color = season), 
      size = 0.5, alpha = 0.6) +
    ...
    facet_grid(
      ...
      labeller = label_wrap_gen(width = 14)) +
    labs(
      y = "Bray-Curtis Dissimilarity to 1970",
      x = "Year", color = "Season")
