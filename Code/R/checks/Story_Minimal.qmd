...
## E. Size Distribution Percentiles 
...
### Dogfish Impact on Seasonal Differences
...
```{r}
#| label: dogfish removal

# Separate axes for 90
quants_10 <- wigley_season_qs %>% 
  filter(Percentile == "10%") %>% 
  mutate(Percentile = "10th Percentile")
quants_90 <- wigley_season_qs %>% 
  filter(Percentile == "90%") %>% 
  mutate(Percentile = "90th Percentile")
spiny_10 <-  spiny_season_qs %>% 
  filter(Percentile == "10%") %>% 
  mutate(Percentile = "10th Percentile")
...
```{r}
#| label: drop single key species
#| eval: false

# # Drop a single species
# drop_species <- "atlantic herring"
# oneout_season_qs <- bind_rows(
#     mutate(no_spiny, area = "Northeast Shelf"),
#     no_spiny) %>% 
#   filter(comname != drop_species) %>% 
#   unite(col = "group_var", area, est_year, season, sep = "X", remove = T) %>% 
#   split(.$group_var) %>% 
#   map_dfr(function(x,y){
#     
...
#     data = oneout_10,
#     aes(x = est_year, y = kg_roll, 
#         fill = str_c("Removed ", drop_species)),
#     alpha = 0.6, color = "gray20") +
#   scale_fill_manual(values = c("gray70", "gray20")) +
#   scale_x_continuous(expand = expansion(add = c(0,0))) +
#   scale_y_continuous(labels = label_number(suffix = "kg")) +
