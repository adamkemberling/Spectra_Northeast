...
mismatch_ranks <- function(x){
  species_summaries <- x %>% 
    group_by(est_towdate, est_year, comname, id, season, catchsex) %>% 
    summarise(
  ...
  tow_totals <- x %>% 
    distinct(est_towdate, est_year, season, station, id,  comname, catchsex, abundance, biomass_kg) %>% 
    group_by(est_towdate, est_year, comname, id, season, catchsex) %>% 
    summarise(
...
### Are Dogfish Biases Sex or Season Specific
