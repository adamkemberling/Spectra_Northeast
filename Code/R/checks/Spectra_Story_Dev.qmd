...
I lack confidence that the values we have for slopes presently:

 * Represent the actual distribution of the data (mis-application of an assumed pareto distribution)   
 * Are representative of some meaningful feature of the community (local changes in slope over some range of body-sizes has less meaning than what has been ascribed to it)    
 
 I also don't know whether slopes are comparable across regions. This last point is more important salient when comparing absolute values shift when min/max parameters change (below).
...
wigley_bmspectra_model_df %>% 
  ...
  coord_flip() +
  labs(
    y = "Exponent of Individual Body Mass Distribution", 
    x = "",
    title = "Exponent of Individual Mass Distribution (1g - Max)")
...
In all regions it seems like the typical linear slope begins at a larger size than 1g. If we take a year at random the distributions are even more patchy AND we may potentially introduce situations where the min-max size range is quite small (no large fishes sampled).
...
The length distributions look even more flat than the biomass distributions, basically ending at the 128-256cm bin.
...
After doing some attempts to plot the impacts of moving it around, I am less convinced that its helping things and it may even be harming the situation as the range between min/max sizes can become quite small. 
...
With respect to trends, we lose the fall steepening ocurring in the Gulf of Maine, otherwise the direction of trends remain the same.
...
Before I plotted this I was concerned about the absolute values shifting in a meaningful way. I'm less concerned now.
...
ggplot(aes(fct_rev(area_titles), b, color = season, fill = season)) +
  ...
  labs(
    y = "Exponent of Individual Body Mass Distribution", 
    x = "",
    title = "Exponent of Individual Mass Distribution (16g - Max)")
...
We may want to drop some outliers there.
...
wigley_bmspectra_model_df %>% 
  ...
  ggplot(aes(fct_rev(area_titles), size_range,  color = season, fill = season)) +
  ggdist::stat_halfeye(
     alpha = 0.4,
     adjust = .5, 
     width = .6, 
  ...
  labs(
    title = "Variability in the Size Range for Spectra Estimate",
    x = "",
...
Everything below might have a bug so don't obsess over these fit plots for the time being.
