...
#   mutate(size_thresh = if_else(Index == "LFI", ">90th Percentile\nSpecies-Based", "<10th Percentile\nSpecies-Based")) %>%
#   ggplot(aes(est_year, val, color = season)) +
#   geom_point(
#     aes(est_year, val, color = season),
#     size = 0.8, alpha = 0.5) +
#   geom_ma(
...
#   facet_nested(survey_area~fct_rev(Index)*size_thresh) +
#   labs(
#     x = "Year",
...
```{r}
#| label: wigley weighted percentiles
...
#   scale_color_gmri() +
#   scale_y_continuous(labels = label_percent()) +
#   facet_nested(survey_area~fct_rev(Index), scales = "free_y") +
#   labs(
#     x = "Year",
#     y = "Percent of Overall Abundance",
#     title = "Wigley species, Large and Small Fish Indices, by Weight")
...
```{r}
#| label: plot wigley regional lfi
...
Using these broad group quantiles, we have the following species are represented within the large fish index:
...
### Wigley Species-Specific LFI

Using species-specific quantiles shifts the story somewhat:
...
### Shift in Regional Percentile Size
...
(q10_p | q90_p | q95_p) + plot_layout(guides = "collect", axes = "collect_x") +
  plot_annotation(title = "Size Percentile Changes")
