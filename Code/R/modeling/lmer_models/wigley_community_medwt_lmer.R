...
# 2. weight spectra model df
landings_temp_lm <- lm(landings ~ bot_temp, data = wtb_model_df)
...
ggplot(wtb_model_df, aes(yr_num, b, color = season)) +
  ...
  facet_wrap(~survey_area, scales = "free") +
  scale_color_gmri() +
  labs(title = "Weight Spectra Slope - MLE Bins Method",
       subtitle = "Finfish Community\nEnforced xmin = 1, xmax = max(Weight_kg + 1)",
       y = "b",
       x = "Year",
       color = "Season")
...
wig_wt_mod <- lmerTest::lmer(
  #formula = med_wt_kg ~ survey_area * yr_num * season + (1 | yr_fac),
...
RandomEffects <- as.data.frame(VarCorr(wig_wt_mod))
...
wig_wt_mod2 <- lmerTest::lmer(
  formula = log(med_wt_kg) ~ survey_area * season + log10(landings) + scale(bot_temp) + (1 | yr_fac),
  data = wt_model_df)
...
RandomEffects <- as.data.frame(VarCorr(wig_wt_mod2))
ICC_between <- RandomEffects[1,4]/(RandomEffects[1,4]+RandomEffects[2,4]) 
ICC_between
# 4.2%
...
bt_preds <- as.data.frame(
  ggpredict(wig_wt_mod2, ~ bot_temp + survey_area + season) )
...
(sc_p2_regionseason_emmeans <- regseas_phoc_wt2$`emmeans of survey_area, season` %>%
    as_tibble() %>%
    #ggplot(aes(survey_area, emmean, ymin = lower.CL, ymax = upper.CL)) +
    # next two are the same, adjusting for log(weight)
    # ggplot(aes(survey_area, exp(emmean), ymin = exp(lower.CL), ymax = exp(upper.CL))) +
    ggplot(aes(survey_area, response, ymin = lower.CL, ymax = upper.CL)) +
    ...
    labs(
      ...
      title = "Small Finfish Community",
      color = "Season",
      subtitle = "Median Weight Region x Season Fixed Effects - Post-Hoc"))


# Save
ggsave(
  plot = sc_p2_regionseason_emmeans, 
  filename = here::here("Figs/small_community/sc_medwt_regionseason_emmeans.png"))
