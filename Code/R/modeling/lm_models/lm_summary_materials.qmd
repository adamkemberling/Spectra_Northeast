...
manual_colors <- c(
  "Well-Studied Community" = gmri_cols("blue"),
  "Data-Limited Community" = gmri_cols("orange")
)
```
...
Each table below displays the group level significance, as well as a predictor level significance from an F-Test is shown (uses car::Anova undr the hood).
...
# # Coefficient Table
# emmeans(sc_len$pass_2, specs = ~bot_temp)
# len_temp_trends %>%
#   mutate_if(is.numeric, ~round(.x, 3)) %>%
#   group_by(survey_area) %>% 
#   gt() %>% 
#   tab_header(title = "Length Bottom Temp EMTrends")
